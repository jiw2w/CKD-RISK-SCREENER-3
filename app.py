...
if model and bundle:
    ...
    try:
        # CalibratedClassifierCV로 감싸진 경우와 그렇지 않은 경우를 모두 처리
        if hasattr(model, 'estimator'):
            base_stacking_model = model.estimator
        else:
            base_stacking_model = model
        
        trained_ebm_in_stack = base_stacking_model.named_estimators_['ebm']
        ebm_local = trained_ebm_in_stack.explain_local(X_new, [int(pred_prob > threshold)])
